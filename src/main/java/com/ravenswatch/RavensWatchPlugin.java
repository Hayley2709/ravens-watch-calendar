...
public class RavensWatchPlugin extends Plugin
{
    ...
    public void onChatMessage(ChatMessage event)
    {
        ...
        switch (event.getType())
        {
            ...
            case CLAN_GIM_MESSAGE:
                ...
                // Filters messages so it only logs your local player's events, matches broadcast criteria, and hits value threshold
                if (isBroadcastableMessage(message) && isLocalPlayerEvent(message) && meetsValueThreshold(message))
                {
                    sendDiscordEmbedWebhook(config.clanWebhookUrl(), message);

                    if (panel != null)
                    {
                        panel.addRecentDrop(message);
                    }
                }
                break;
            default:
    ...
    private boolean meetsValueThreshold(String message)
    {
        // If the message is a pet, level up, quest, or collection log unlock, it doesn't have a coin value, so always allow it
        String lower = message.toLowerCase();
        if (lower.contains("pet") || lower.contains("collection log unlocked") || lower.contains("feels like you're being followed") ||
                lower.contains("level") || lower.contains("xp in") || lower.contains("completed") || lower.contains("personal best"))
        {
            return true;
        }

        // Try to parse values out of parentheses like "(1,200 coins)" or "(1,200,000 coins)"
        try
        {
            java.util.regex.Pattern pattern = java.util.regex.Pattern.compile("\\(([\\d,]+)\\s+coins?\\)", java.util.regex.Pattern.CASE_INSENSITIVE);
            ...
            if (matcher.find())
            {
                ...
                return dropValue >= config.minimumDropValue();
            }
        }
        catch (Exception e)
        {
            log.error("Failed to parse drop value from message: {}", message, e);
        }
